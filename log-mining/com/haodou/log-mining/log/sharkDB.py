...
class SharkDB:
	...
	def queryString(self, sql):
		...
		cmd = """\
		/opt/app/shark/bin/shark -skipRddReload -i /home/zhangzhonghui/hiv/hiverc -e "%s" \
		"""
		#print cmd%sql
		...
		regex = "^[\s\S]*Hive history.+?txt[\s\S]OK(?P<res>[\s\S]*)Time taken[\s\S]*$"
...
if __name__=="__main__":
	db=SharkDB()
	#db.execute("use hd_haodou_recipe_20140720;")
	#ret=db.execute("select * from limit 100;")
	ret=db.execute("show databases;")
	for line in ret:
		print line
