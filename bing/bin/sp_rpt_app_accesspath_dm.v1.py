...
c_session_seq, \
c_version_id , \
c_function_id = range(outfieldnum)
...
"%(node_function)s" + fieldsdelimiter + \
...
select app_id, device_id, channel_id, session_id, session_seq,
version_id, function_id
from bing.dw_app_interaction_session_dm
where statis_date='${statis_date}'
and device_id!='' and session_seq<=50
distribute by app_id, device_id
sort by app_id, device_id, channel_id, session_id, session_seq
;
"""
...
resultset['NULL']['node_function'] = 'NULL'
...
for outline in hive.stdout:
  ...
  if str(outrow[c_session_seq])=='1':
    ...
    cur_node_function = outrow[c_function_id]
    cur_node_path = cur_node_function
    ...
    cur_version_id = outrow[c_version_id]
    cur_node_function = outrow[c_function_id]
    cur_node_path = cur_node_path + ':' + cur_node_function
    cur_node_key = hashlib.md5(cur_node_path).hexdigest()
  ...
  if curkey not in resultset:
    ...
    resultset[curkey]['node_function'] = cur_node_function
...
for key in resultset:
  ...
  resultline = resultrowformat % { \
    ...
    'node_function':resultset[key]['node_function'], \
...
##write to tmpfile
logger.debug("write to datafile ...")
