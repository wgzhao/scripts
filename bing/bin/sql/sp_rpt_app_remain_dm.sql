...
--不分版本留存
insert overwrite table bing.rpt_app_remain_dm_np partition (exec_date='${statis_date}')
select 
...
from (
select /*+ mapjoin(ds)*/
ds.statis_date, ds.app_id, ds.device_id, ds.version_id, dm.active_date, 
datediff(dm.active_date,ds.statis_date) as remain_day
from
(select app_id, first_version as version_id, device_id, to_date(first_day) as statis_date
...
group by statis_date, app_id
;

--分版本留存
insert into table bing.rpt_app_remain_dm_np partition (exec_date='${statis_date}')
select 
...
group by statis_date, app_id, version_id
;

--动态分区写入结果表
insert overwrite table bing.rpt_app_remain_dm partition (statis_date)
select 
app_id         ,
version_id     ,
new_devnum     ,
...
remain_devnum30,
statis_date    
from bing.rpt_app_remain_dm_np
