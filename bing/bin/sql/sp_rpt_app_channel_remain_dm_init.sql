...
--不分版本留存
insert overwrite table bing.rpt_app_channel_remain_dm partition (statis_date='${statis_date}')
select 
...
from (
select /*+ mapjoin(ds)*/
ds.app_id, ds.device_id, ds.channel_id, ds.version_id, dm.active_date, 
datediff(dm.active_date,to_date('${statis_date}')) as remain_day
from
(select app_id, first_channel as channel_id, first_version as version_id, device_id
...
--分版本留存
insert into table bing.rpt_app_channel_remain_dm partition (statis_date='${statis_date}')
select 
