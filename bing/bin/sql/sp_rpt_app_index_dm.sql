...
insert overwrite table bing.rpt_app_index_dm partition (statis_date='${preday_date}')
select app_id, version_id, index_name, index_value
from bing.rpt_app_index_dm
where statis_date='${preday_date}' and index_name!='日留存用户数'
;

insert into table bing.rpt_app_index_dm partition (statis_date='${preday_date}')
select app_id, '*' as version_id, '日留存用户数' as index_name, count(device_id) as index_value
from bing.dw_app_device_remain_ds
where first_day between '${preday_date} 00:00:00' and '${preday_date} 23:59:59'
and substr(remainlog,1,1)='1' and app_id in ('1','2','3','4','5','6')
group by app_id
;

insert into table bing.rpt_app_index_dm partition (statis_date='${preday_date}')
select app_id, first_version as version_id, '日留存用户数' as index_name, count(device_id) as index_value
from bing.dw_app_device_remain_ds
where first_day between '${preday_date} 00:00:00' and '${preday_date} 23:59:59'
and substr(remainlog,1,1)='1' and app_id in ('1','2','3','4','5','6')
