...
accessfilename = 'accesslog_%(statisdate)s.dat' % {'statisdate':statisdate}
accessfile = '%(dir)s/%(filename)s' % {'dir':tmpdir, 'filename':accessfilename}
accesswriter = open(accessfile,'w+')
...
applist = ['2','4','6']
accessact, ignorepage, accesscnt = ['A1000','A1002'], ['BlankActivity'], 0
  ...
  for extinfo in extlist:
    ...
    if cur_action in accessact:
      ...
      if cur_page!=page:
        ...
        if accesscnt % chunksize == 0:
          try:
            accesswriter.writelines(resultchunk)
            resultchunk = []
          except:
            accesswriter.close()
hdfs.terminate()
logger.debug("statistics info (app_action.log)\n rownum: %d\n errnum: %d" % (rownum,errnum))
logger.debug(" access count: %d" % (accesscnt))

accesswriter.writelines(resultchunk)
accesswriter.close()

logger.debug("zipping file ...")
if os.path.exists(accessfile+".lzo"):
  runShell("rm %(tmpfile)s.lzo" % {'tmpfile':accessfile})
runShell("cd %(tmpdir)s && lzop %(tmpfilename)s" % {'tmpdir':tmpdir, 'tmpfilename':accessfilename})

logger.debug("loading data ...")
runHiveQL("load data local inpath '%(tmpfile)s.lzo' overwrite into table bing.ods_app_accesslog_dm partition (statis_date='%(statis_date)s');" % {'tmpfile':accessfile, 'statis_date':statis_date})

logger.debug("deleting tmpfile ...")
runShell("cd %(tmpdir)s && ls -l %(tmpfilename)s* | grep -v .lst > %(tmpfilename)s.lst" % {'tmpdir':tmpdir, 'tmpfilename':accessfilename})
runShell("rm %(tmpfile)s" % {'tmpfile':accessfile})
runShell("rm %(tmpfile)s.lzo" % {'tmpfile':accessfile})
