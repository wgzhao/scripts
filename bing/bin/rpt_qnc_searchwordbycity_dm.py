...
sql = ''
with open(os.path.split(os.path.abspath(__file__))[0] + '/sql/sp_rpt_qnc_searchwordbycity_dm.sql', 'rb') as f:
    s = f.read()
    sql = s.replace('${statis_date}', statis_date) \
        .replace('${statisdate}', statisdate) \

cmd = """/opt/app/spark/bin/spark-sql --jars hdfs://hdcluster/udf/haodoubihiveudf.jar -f /tmp/lifangxing/sp_rpt_qnc_searchwordbycity_dm.sql"""

#print sql
if not os.path.exists('/tmp/lifangxing'):
    os.mkdir('/tmp/lifangxing')
with open('/tmp/lifangxing/sp_rpt_qnc_searchwordbycity_dm.sql', 'w') as f:
    f.write(sql)
